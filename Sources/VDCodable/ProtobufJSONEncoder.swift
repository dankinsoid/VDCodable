// Sources/ProtobufJSONEncoder.swift - JSON Encoding support
//
...
internal struct ProtobufJSONEncoder {
	...
	internal mutating func putKey(_ name: String) {
		data.append(asciiDoubleQuote)
		// Can avoid overhead of putStringValue, since
		// the JSON field names are always clean ASCII.
		data.append(contentsOf: name.utf8)
	...
	internal mutating func openSquareBracket() {
		data.append(asciiOpenSquareBracket)
	}
	
	/// Append a close curly brace `}` to the JSON.
	internal mutating func closeSquareBracket() {
		data.append(asciiCloseSquareBracket)
	}
	...
	internal mutating func openCurlyBracket() {
		data.append(asciiOpenCurlyBracket)
	}
	
	/// Append a close curly brace `}` to the JSON.
	internal mutating func closeCurlyBracket() {
		data.append(asciiCloseCurlyBracket)
	}
